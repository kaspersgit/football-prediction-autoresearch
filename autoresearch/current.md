...
10. **Re-test Pinnacle odds as continuous model features (not just the binary confirmation filter), and separately test an opening/closing average.** `EXP-20260420-S037` already tried this once (`pinnacle_h/d/a` added alongside and in place of `market_h/d/a`) and reverted on regression — but that ran on a much earlier pipeline (global model, pre-per-league, pre-confirmation-filter, and a 35–40% Pinnacle null rate that season). Current coverage is far better (94.8% joint `PSH/PSD/PSA`+`PSCH/PSCD/PSCA`, per `EXP-20260810-003`'s pre-check), and the confirmation filter now exists as a *binary* veto — a continuous feature could capture confirmation *magnitude* that the hard threshold discards, which is different information from what `S037` tested, not a re-run of it.
    - **Two sub-questions to keep separate**, since they have different risk profiles and different paths to production:
      1. **Pinnacle closing odds as a feature.** Candidate: `pinnacle_fair − b365_fair` margin per outcome (not raw `pinnacle_h/d/a`, which duplicates `market_h/d/a` — avoid repeating `S037`'s Variant A collinearity mistake). Inherits the same look-ahead-optimism already documented for the filter (`EXP-20260810-004`, active hypothesis 5) — closing odds aren't observable at live-prediction time, so a positive backtest result here is bounded by the same open question about how well live-fetched odds track closing-line behavior. Not a new leakage problem, but a deeper reliance on an already-flagged one.
      2. **Avg(opening, closing) as a feature.** Two distinct things this could mean — test and report them separately, don't conflate:
         - *Backtest-only smoothing*: average the historical `PSH/PSD/PSA` and `PSCH/PSCD/PSCA` columns as a diagnostic for whether smoothing reduces noise vs. pure closing. Has no path to production as specified (still a function of the unobservable closing line).
         - *Live-computable variant*: average two snapshots actually fetched via The Odds API at different times before kickoff (e.g. day-before poll + near-kickoff poll). Genuinely deployable, but requires redesigning `predict.sh`'s single-fetch flow into two stages — separate engineering effort from the feature-column change itself, only worth it if the backtest smoothing test shows promise.
    - **Before trusting either as new signal**, correlation-check against `market_bias`/`market_overround` — same precedent as item 9 (`EXP-20260426-D055` found a feature fully derivable from existing market features). Also worth weighing against `EXP-20260810-003`: a related but distinct transform (each team's own historical opening→closing movement, lagged) net-regressed, which is weak prior evidence against the movement/blending family generally, though not dispositive for a same-match average.
    - Bar for keeping: standard per-league walk-forward evaluation per `EVALUATION.md` — ROI/stability improvement, not just accuracy.
