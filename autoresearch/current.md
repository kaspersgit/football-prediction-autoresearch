...
# Overreaction / market-bias diagnostics (all standalone, no training) -- EXP-2026090[9,10]
uv run python main.py --hot-hand-diagnostic        # EXP-20260909-001 (rejected)
uv run python main.py --cod-diagnostic             # EXP-20260910-001 (feature reverted)
uv run python main.py --rest-diagnostic            # EXP-20260910-003 (rejected)
uv run python main.py --triads-diagnostic          # EXP-20260910-004 (rejected)

# Pinnacle-confirmation margin sweep over the production portfolio (one train, then sweep)
uv run python main.py --pinnacle-margin-sweep      # EXP-20260910-005
...
4. ~~Fix `main.py:_run_compare_vig`'s per-league breakdown `KeyError: 'league'`.~~ **DONE — KEPT, `EXP-20260910-002` (2026-09-10).** The bet DataFrame already carries `league` (from `compute_value_betting_results`); the redundant re-merge against `eval_df` collided (`league_x`/`league_y`). Extracted to module-level `_compare_vig_league_roi`, reads the column directly; `--compare-vig` runs end-to-end again. Regression test added.
...
10. ~~Pinnacle-confirmation margin.~~ **DONE — 0.015 CONFIRMED, no change, `EXP-20260910-005` (2026-09-10).** Formal sweep (`--pinnacle-margin-sweep`, new tool): margin ∈ {none, 0.0, 0.005, 0.010, **0.015**, 0.020, 0.025, 0.030} → ROI {−0.9%, +12.3%, +12.4%, +12.5%, **+17.7%**, +18.5%, +20.0%, +18.6%} / t {−0.35, +3.13, +2.92, +2.69, **+3.42**, +3.20, +3.14, +2.67} / bets {3295, 1537, 1279, 1024, **821**, 637, 483, 376}. 0.015 has the highest t-stat and near-highest ROI with every production league positive; lowering it fails the volume-for-ROI rule's ≤5pp ROI-decline cap (0.005/0.010 both breach it); raising it sheds volume for marginal ROI. No code change beyond the sweep tool. (Supersedes the informal 2026-08-29 sweep; absolute numbers shifted with more current-season data but the curve shape held.)

11. ~~COD season-luck quantile (Wheatcroft 2020).~~ **DONE — diagnostic borderline, feature REVERTED, `EXP-20260910-001` (2026-09-10).** Step-1 diagnostic (`--cod-diagnostic`, kept as a tool) found a *real, monotone* market bias — ROI of backing a side falls cleanly from +0.0% (lowest-COD decile) to −8.6% (highest, t −5.6), and fading the higher-COD side vs the lower-COD side has paired t +2.80 — but the low-COD long side is only break-even against real prices, and it is negative in `N1`/`G1` (profitable pockets are B1 +17.5%/t2.3 and F1, i.e. exactly Wheatcroft's leagues; the plan's ≥2-of-`N1/P1/B1` gate is met in only B1). Step 2 tested anyway: `cod_diff` in `FEATURE_COLS` made all-market ROI −3.26%→−5.31%, stability and t-stat both worse, 2/11 leagues improved → reverted; `features.py` byte-identical to before. Overlaps `market_bias`.
...
13. **Lower-priority leads from the 2026-09-09 literature scan:**
    - ~~Non-transitive triads (van Ours 2025).~~ **DONE — REJECTED (soft), `EXP-20260910-004`.** `--triads-diagnostic` (kept as a tool): with a loose "any third team forms a cycle" definition, 64% of matches flag; backing the market underdog is −8.2% in-triad vs −9.7% not-in-triad, gap +1.6pp at Welch t +0.77 (noise). Only E0 non-negative (+1.6%), weakly consistent with the paper's EPL focus. Revisit only with a much tighter triad definition (third team a common recent opponent; restrict to near-pick'em matches).
    - ~~Asymmetric rest disadvantage.~~ **DONE — REJECTED, `EXP-20260910-003`.** `--rest-diagnostic` (kept as a tool): 137–369 qualifying spots across all thresholds (cross-competition congestion is invisible in single-league data), backing the rested underdog loses 9–22% and the paired t is *negative* at every cut — the domestic market prices short rest fine.
    - **Asian-Handicap-implied 1X2 as the edge baseline** (from *Forecasting soccer matches with betting odds: a tale of two markets*, Int. J. Forecasting 2025, [ScienceDirect](https://www.sciencedirect.com/science/article/pii/S0169207024000670)): AH markets show no favourite-longshot bias and concentrate sharp money; a 1X2 fair prob derived from AH + totals lines may be a cleaner `baseline_prob` than vig-stripped B365 1X2. Infrastructure change (better baseline that every downstream edge calc inherits), not a feature. football-data.co.uk carries AH/totals columns in many files — check coverage across our leagues/seasons first. **Still open.**
    - **xG mean reversion** (Wilkens, *Can simple models predict football*, 2026, [link](https://journals.sagepub.com/doi/10.1177/22150218261416681); ~10% avg-odds / ~15% best-odds ROI on Bundesliga): strongest single academic lead but blocked on data — Understat/FBref xG covers the big-5 leagues, not `N1/P1/G1`. Already tracked as item 3; supporting evidence for why it's worth doing *if* the production set ever changes or as an evaluation-only signal. **Still open (blocked on data).**
...
Cleared 2026-09-10 (five iterations): item 11 COD season-luck — diagnostic found a real monotone bias but not a bettable/feature-able one, `cod_diff` feature reverted (`EXP-20260910-001`); item 4 `_run_compare_vig` crash fixed and kept (`EXP-20260910-002`); item 13 asymmetric rest rejected (`EXP-20260910-003`) and non-transitive triads rejected-soft (`EXP-20260910-004`); item 10 Pinnacle margin swept and 0.015 confirmed unchanged (`EXP-20260910-005`). Four new standalone diagnostic tools kept (`--cod-diagnostic`, `--rest-diagnostic`, `--triads-diagnostic`, `--pinnacle-margin-sweep`), all isolated from the model/backtest/predict paths. Item 13's AH-implied-1X2 baseline and xG (item 3) sub-items remain open; items 12 (sharp-line structure) untouched.
