...
## EXP-20260828-002: Add missing league one-hot features (league_G1/SC0/B1/T1) — KEPT (correctness fix, verified no-op on production/research paths)

**Date:** 2026-08-28
**Hypothesis:** While auditing the codebase for other places Greece (G1) might be missing after `EXP-20260828-001`, found `src/model/features.py`'s `FEATURE_COLS` one-hot-encodes league identity as `league_E0, league_D1, league_SP1, league_F1, league_N1, league_P1` (I1 as the implicit dropped reference) — a list predating G1/SC0/B1/T1 even being added to `SUPPORTED_LEAGUES`. Initial hypothesis: since `PRODUCTION_LEAGUES` has included G1 since this project's first commit of that config, every model ever trained for live Greece predictions has been missing a Greece-identity signal relative to every other production league.

**Files changed:** `src/model/features.py` (`FEATURE_COLS` now derived from `SUPPORTED_LEAGUES` instead of hardcoded, at both the training-data and live-fixture feature-building call sites), `tests/test_features.py` (new regression test asserting `FEATURE_COLS` has a `league_<code>` column for every `SUPPORTED_LEAGUES` entry except the reference).

**Result — verified as a true no-op, not just "small":** Ran the standard research command (`--per-league --threshold 0.0 --pinnacle-filter --update`) via a one-off `Evaluate` workflow run (throwaway branch, workflow run 33215179506) and compared against `EXP-20260828-001`'s baseline. Every number came back **bit-for-bit identical**: all-market accuracy 0.519, ROI −4.67%, stability −0.0310, t-stat −2.96, identical `ROI BY LEAGUE` table for all 11 leagues, identical calibrated thresholds, and identical 5-league production portfolio (748 bets, +15.59% ROI, same per-league split to two decimal places). The CI job's own artifact-diff check (`git diff --staged --quiet` before the "update evaluation artifacts" commit) found **zero changes** to `reports/backtest_bets.csv`/`evaluation_bets.csv`/`models/league_thresholds.json` — independent confirmation, not just visually-similar output.

**Root cause of the no-op — the original hypothesis was wrong about *why* this mattered:** Both the live production pipeline (`train_on_all_data_per_league`) and the standard research methodology (`--per-league`, per `EVALUATION.md`) train **one separate model per league**, each fit only on that league's own rows (`src/model/train.py`: `l_train = train_mask & (full_odds["league"] == league)`). Within any single league's training subset, every `league_<code>` one-hot column is **constant** (1 for that league's own model, 0 for every other league's model) and therefore carries zero split information for a tree model — LightGBM never selects a zero-variance column. This was already true for the *original* six columns (`league_E0` etc.) before this change, not just the four missing ones: under per-league training, no league has ever gotten anything from these features, Greece included. So the live-impact framing from `EXP-20260828-001`'s follow-up write-up ("every model ever trained for live Greece predictions has been missing a Greece-identity signal") is corrected here — no league was ever advantaged over Greece by this omission. The columns are only non-constant, and only actually useful, in the single shared multi-league model path (`train_walkforward(per_league=False)`, i.e. plain `uv run python main.py` with no `--per-league` flag) — a secondary/legacy mode not used by production or the standard evaluation.
