...
def test_attach_pinnacle_odds_rejects_a_different_matchweek(monkeypatch):
    """Same team pairing, but the live odds are for a different round than fixtures.csv."""
    fixtures_df = pd.DataFrame({
        "Date": pd.to_datetime(["2026-08-10"]),
        "HomeTeam": ["Utrecht"],
        "AwayTeam": ["Twente"],
    ...
    assert pd.isna(result.loc[0, "PSH"])
    assert pd.isna(result.loc[0, "PSD"])
    assert pd.isna(result.loc[0, "PSA"])
